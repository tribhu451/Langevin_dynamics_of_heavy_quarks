...
int main() {
  ...
  int Nevents = 100000 ; // No of PYTHIA events to be generated.
  ...
  int index_of_primary_cs[200] ;
  int mother1_of_primary_cs[200] ;
  int mother2_of_primary_cs[200] ; 
  int index_of_final_product_cs[200] ; 
  int pairing_flag_of_cs[200] ; 
  int final_product_counter_flag_of_cs[200] ; 
  ...
  for (int iEvent = 0; iEvent < Nevents; ++iEvent) {
    ...
    for(int ii=0; ii<counter_primary_cs; ii++){
      ...
      final_product_counter_flag_of_cs[ii] = 0 ;  
      int counter = 0 ; 
	...
	if( mother1_of_primary_cs[ii] == mother1_of_primary_cs[jj] &&
	  ...
	  for(int ll=0; ll<produced_charm_quark_number; ll++){
	    if( produced_charm_quark_index[ll] == index_of_primary_cs[jj] ){
          ...
          if(produced_charm_quark_pid[f1] == produced_charm_quark_pid[f2]){
            std::cout << " ************** [Tribhuban Message] Not a true pair."
                      << " Hence not writting in file" << std::endl ;
            continue ;  
          }
